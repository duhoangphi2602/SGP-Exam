...
public class DangKyThiController {
	...
	private String kiemTraSoCau(String maMH, String trinhDo, int soCauThi) {
		...
		if (cauChinh < cauChinhToiThieu)
			return String.format("Không đủ câu hỏi trình độ %s! Hiện có: %d câu, cần thêm: %d câu", trinhDo, cauChinh,
					cauChinhToiThieu - cauChinh);
		if (tongCauCoThe < soCauThi)
			return String.format("Không đủ câu hỏi! Tổng có thể lấy: %d câu, cần thêm: %d câu", tongCauCoThe,
					soCauThi - tongCauCoThe);
		return null;
	}

	// =====================================================
	// Trang chính — hiện form + danh sách (THAY THẾ index cũ, XÓA showThem/showSua
	// GET)
	// =====================================================
	@RequestMapping(value = "/dangkythi.htm", method = RequestMethod.GET)
	public String index(@RequestParam(required = false) String maLop, @RequestParam(required = false) String maMH,
			@RequestParam(required = false) Integer lan, HttpSession session, Model model) {
	...
	public String getClassRegistrations(@RequestParam("maLop") String maLop) {
		...
		for (int i = 0; i < list.size(); i++) {
			...
			json.append(String.format("{\"maMH\":\"%s\", \"lan\":%s, \"ngayThi\":\"%s\"}", map.get("maMH"),
					map.get("lan"), map.get("ngayThi")));
			if (i < list.size() - 1)
				json.append(",");
		}
		json.append("]");
	...
	public String getMaxQuestions(@RequestParam("maMH") String maMH, @RequestParam("trinhDo") String trinhDo) {
		// Bên trong giữ nguyên y xì cũ
		if (maMH == null || maMH.trim().isEmpty() || trinhDo == null || trinhDo.trim().isEmpty())
			return "{\"max\": 0}";
