	...
	public String doLogin(@RequestParam String username, @RequestParam String password, @RequestParam String loai,
	    ...
	    try {
	        ...
	        if (loai.equals("SINHVIEN")) {
	            ...
	            //  Lấy họ tên từ object sv đã query sẵn
	            session.setAttribute("hoTen", sv.getHo().trim() + " " + sv.getTen().trim());
	            ...
	            // ✅ Lấy họ tên thật qua SP_GV_GETBYMA
	            List<Map<String, Object>> gvInfo = db.queryForList("EXEC SP_GV_GETBYMA ?", dbUsername);
	            if (!gvInfo.isEmpty()) {
	                String ho = gvInfo.get(0).get("HO").toString().trim();
	                String ten = gvInfo.get(0).get("TEN").toString().trim();
	                session.setAttribute("hoTen", ho + " " + ten);
	            } else {
	                // Fallback nếu không tìm thấy (ví dụ tài khoản PGV không có trong bảng GV)
	                session.setAttribute("hoTen", username);
	            }
