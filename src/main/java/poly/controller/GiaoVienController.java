...
public class GiaoVienController {
	...
	private String validateGV(GiaoVien gv, String maGVLoaiTru) {
		...
		if (!gv.getMaGV().trim().matches("[a-zA-Z0-9]+")) {
			return "Mã giáo viên không được chứa khoảng trắng hoặc ký tự đặc biệt!";
		}
		if (gv.getHo() == null || gv.getHo().trim().isEmpty() || gv.getTen() == null || gv.getTen().trim().isEmpty()) {
			return "Họ và Tên không được để trống!";
		}
		String hoTenPattern = "[\\p{L}\\s]+";
		if (!gv.getHo().trim().matches(hoTenPattern) || !gv.getTen().trim().matches(hoTenPattern)) {
	...
	private String buildRows(List<GiaoVien> list) {
		...
		for (GiaoVien gv : list) {
			...
			sb.append("<button type=\"button\" class=\"btn btn-sm btn-warning\" ").append("onclick=\"moModalSua('")
					.append(escapeJs(gv.getMaGV().trim())).append("','").append(escapeJs(gv.getHo().trim()))
					.append("','").append(escapeJs(gv.getTen().trim())).append("','").append(escapeJs(gv.getSoDTLL()))
					.append("','").append(escapeJs(gv.getDiaChi())).append("')\">Hiệu chỉnh</button> ");
			sb.append("<button type=\"button\" class=\"btn btn-sm btn-danger\" ").append("onclick=\"xoaGV('")
					.append(escapeJs(gv.getMaGV())).append("')\">Xóa</button>");
			sb.append("</td>");
			sb.append("</tr>");
		}
		return sb.toString();
	...
	@ResponseBody
	public String doGhi(@RequestParam("maGV") String maGV, @RequestParam("ho") String ho,
			...
			@RequestParam("mode") String mode, HttpSession session, HttpServletResponse response) {
		response.setContentType("text/plain;charset=UTF-8");
		try {
			GiaoVien gv = new GiaoVien();
			gv.setMaGV(maGV.trim());
			gv.setHo(ho.trim().toUpperCase());
			gv.setTen(ten.trim().toUpperCase());
	...
	public String doPhucHoi(HttpSession session, HttpServletResponse response) {
		...
		try {
			...
			giaoVienDAO.phucHoi(action.getLoai(), data.getMaGV(), data.getHo(), data.getTen(), data.getSoDTLL(),
					data.getDiaChi());
			return "OK|" + buildRows(giaoVienDAO.findAll());
		} catch (Exception e) {
