	...
	<div class="container mt-4">
		...
		<div class="card shadow-sm mb-4">
			<div class="card-body bg-light">
				<form action="${pageContext.request.contextPath}/gv/bangdiem.htm"
					method="GET" class="row g-3 align-items-end">
					<div class="col-md-3">
						<label class="form-label fw-bold">Lớp</label> <select name="maLop"
							class="form-select">
					...
					<div class="col-md-2">
						<label class="form-label fw-bold">Lần thi</label> <select
							name="lan" class="form-select">
							<option value="">-- Tất cả --</option>
							<option value="1" ${lanSelected == 1 ? 'selected' : ''}>Lần
								1</option>
							<option value="2" ${lanSelected == 2 ? 'selected' : ''}>Lần
								2</option>
						</select>
					</div>
					<div class="col-md-2">
					...
					<tbody>
						<c:forEach var="ca" items="${dsCaThi}" varStatus="st">
							<tr>
								...
								<td><strong>${ca.MALOP}</strong><br> <small
									class="text-muted">${ca.TENLOP}</small></td>
								<td><strong>${ca.MAMH}</strong><br> <small
									class="text-muted">${ca.TENMH}</small></td>
								<td class="text-center">${ca.LAN}</td>
								<td class="text-center">${ca.NGAYTHI}</td>
								<td class="text-center"><c:choose>
										<c:when test="${ca.TRINHDO == 'A'}">
											<span class="badge bg-danger">A - Chuyên ngành</span>
										</c:when>
										<c:when test="${ca.TRINHDO == 'B'}">
											<span class="badge bg-warning text-dark">B - Không
												chuyên</span>
										</c:when>
										<c:otherwise>
								...
								<td class="text-center">${ca.SOCAUTHI}</td>
								<td class="text-center">${ca.THOIGIAN}phút</td>
								<td class="text-center"><c:choose>
										<c:when test="${ca.SO_DA_THI == 0}">
											<span class="text-muted">0 / ${ca.TONG_SV}</span>
										</c:when>
										<c:when test="${ca.SO_DA_THI == ca.TONG_SV}">
											<span class="text-success fw-bold">${ca.SO_DA_THI} /
												${ca.TONG_SV}</span>
										</c:when>
										<c:otherwise>
											<span class="text-warning fw-bold">${ca.SO_DA_THI} /
												${ca.TONG_SV}</span>
										</c:otherwise>
									</c:choose></td>
								<td class="text-center"><a
									href="${pageContext.request.contextPath}/gv/bangdiem-chitiet.htm?maLop=${ca.MALOP}&maMH=${ca.MAMH}&lan=${ca.LAN}"
